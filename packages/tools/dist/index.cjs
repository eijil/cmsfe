...
var getEnv = (userAgent) => {
  ...
  const uaParser = (0, import_ua_parser_js.UAParser)(userAgent);
...
var ReportSDK = class _ReportSDK {
  ...
  getBaseInfo() {
    ...
    if (!localStorage.getItem("_app_install_id")) {
      const _app_install_id = (0, import_nanoid.nanoid)();
    ...
    if (!sessionStorage.getItem("_app_activate_id")) {
      const _app_activate_id = (0, import_nanoid.nanoid)();
  ...
  async send(data) {
    ...
    const d = import_pako.default.deflate(JSON.stringify([data]));
    const buffer = import_buffer.Buffer.from(d);
