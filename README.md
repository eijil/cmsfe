...
- `@cmsfe/ui`: a stub React component library shared by both `web` and `docs` applications
- `@cmsfe/eslint-config`: `eslint` configurations (includes `eslint-config-next` and `eslint-config-prettier`)
- `@cmsfe/typescript-config`: `tsconfig.json`s used throughout the monorepo
- `@cmsfe/cli` : a CLI tool used to bootstrap new apps and packages
